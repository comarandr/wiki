...
image: https://i.etsystatic.com/66347674/r/il/f3153a/8106445152/il_1588xN.8106445152_qlr5.jpg
mermaid:
  theme: default
---
...
$12 \mod 3 = 0 \qquad -12 \mod 3 = 0$
$13 \mod 3 = 1 \qquad -13 \mod 3 = 2$
$14 \mod 3 = 2 \qquad -14 \mod 3 = 1$
...
$\displaystyle \sum_{k \in K}\ \textbf{c} \cdot a_k = \textbf{c} \cdot \sum_{k \in K}\ a_k$

$\displaystyle \sum_{k \in K}\ (a_k + b_k) = \sum_{k \in K}\ a_k + \sum_{k \in K}\ b_k$

$\displaystyle \sum_{k \in K}\ a_k = \sum_{\Pi (k) \in K}\ a_{\Pi(k)}$

$\displaystyle \sum_{k=1}^\textbf{m}\ a_k + \sum_{k = \textbf{m}}^n\ a_k = a_{\textbf{m}} + \sum_{k=1}^n\ a_k$

$\displaystyle \sum_{k \in K}\ a_k = \sum_{(k + c) \in K}\ a_{(k + c)}$

$\displaystyle \sum_{k = 1}^n\ a_k = \sum_{k = 1}^{\textbf{m}}\ a_k + \sum_{k = \textbf{m+1}}^n\ a_k$
...
| --- | --- | --- |
| per righe | $\displaystyle {\sum_{i=1}^3\ \sum_{j=1}^3\ a_i b_j}$ | $\displaystyle {a_1 \sum_{j=1}^3 b_j + a_2 \sum_{j=1}^3 b_j + a_3 \sum_{j=1}^3 b_j}$ |
| per colonne | $\displaystyle {\sum_{j=1}^3\ \sum_{i=1}^3\ a_i b_j}$ | $\displaystyle {\sum_{i = 1}^3 (a_i)\ b_1 + \sum_{i = 1}^3 (a_i)\ b_2 + \sum_{i = 1}^3 (a_i)\ b_3}$ |

### Scambio di indici

$$\sum_{i \in I} \sum_{j \in J(i)} a_{ij} = \sum_{j \in J} \sum_{i \in I(j)} a_{ij}$$

$${\sum_{i=0}^n \sum_{j=i}^n s_ij} \qquad {\sum_{j=0}^n \sum_{i=0}^j s_ij}$$
...
- $\text{se } p \mid a_1 \cdot a_2 \cdot \ldots \cdot a_r \implies p \vert a_i \left( \text{ almeno un } i \in [1, r]\right)$

**Teorema fondamentale dell'aritmetica**:

$$\forall n \in \mathbb{N}, n \geq 2\\ n = p_1 \cdot p_2 \cdot \ldots \cdot p_i \ \text{ con } p_i = \text{fattore primo}$$

Per cui dati $a,b$ con $\quad a = p_1^{n_1} \cdot p_2^{n_2} \cdot \ldots \cdot p_k^{n_k} \quad$ e $\quad b = p_1^{m_1} \cdot p_2^{m_2} \cdot \ldots \cdot p_k^{m_k}$
...
quante soluzioni intere ha l'equazione $\displaystyle \sum_{i=1}^k x_i = r$ con $x_i \geq b_i\ \forall i \in [1, \ldots, k]$?
...
- $V_i$ : nodi raggiungibili da $i$ senza passare per arco $e$
...
**Esercizio 1**: Dimostra che per $3^{2n} - 1$ è multiplo di $8 \qquad \forall n \geq 0$
