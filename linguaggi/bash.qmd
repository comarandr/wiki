...
### Elencare file di una cartella (listing)  
...
L'informazione è memorizzata in _dischi fissi_ suddivisi in una o più _partizioni_, ognuna delle quali può contenere un file system con una propria _top level directory_.
In UNIX abbiamo un'unica struttura gerarchica.  
...
#### Informarsi sullo spazio occupato
...
`$ tr [stringa1] [stringa2]`  &rarr; i caratteri in stringa1 vengono sostituiti con caratteri corrispondenti in stringa2
...
`s` : substitute
`expr` : stringa da cercare
`new` : stringa da sostituire  
...
### Visualizzare i comandi di uno script
...
### Acccesso e assegnamento variabili in uno script
...
esecuzione dello script contenuto in `./copia`

```bash
$ ./copia nuovadir testo
````

Controllo dell'esecuzione dello script
